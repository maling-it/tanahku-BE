...
import com.tanahkube.repository.RainFallRepository;
...
public class RainFallService {
    ...
    public RainFallService(RainFallRepository rainFallRepository, DeviceRepository deviceRepository){
    ...
    private DataResponse toDataResponse(RainFall data){
    ...
    public WebResponse create(String apiKey ,DataRequest request, Errors errors){
        try {
            ...
            rainFall.setValue(request.getValue());
            rainFall.setDeviceId(request.getDeviceId());
            rainFall.setCreatedOn(new Date());
            rainFall.setIsDelete(false);
            rainFall = this.rainFallRepository.save(rainFall);

            Device device = this.deviceRepository.findById(rainFall.getDeviceId()).orElse(null);
            if (device != null && apiKey.equals(device.getApiKey())) {
                rainFall.setDevice(device);
            }

            DataResponse dataResponse = toDataResponse(rainFall);
    ...
    public WebResponse getLastData(Long deviceId){
        try {
            RainFall rainFall = this.rainFallRepository.findLastData(deviceId).orElse(null);
            if (rainFall != null) {
                DataResponse dataResponse = toDataResponse(rainFall);
    ...
    public WebResponse getLimitData(Long deviceId, Integer limit){
        try {
            List<RainFall> rainFall = this.rainFallRepository.findLimitData(deviceId, limit);
            if (!rainFall.isEmpty()) {
                List<DataResponse> dataResponse = rainFall.stream().map(this::toDataResponse).toList();
                return toWebResponse("success", "success fetch data", dataResponse, new HashMap<>());
            }else{
                return toWebResponse("success", "no data", new HashMap<>(), new HashMap<>());
            }
    ...
    public WebResponse getAllData(Long deviceId, String search, String order, Integer pageSize, Integer pageNumber){
        try {
            ...
            Page<RainFall> dataPage = this.rainFallRepository.findAllData(pageable, deviceId);
            if (!search.trim().equals("")) {
                dataPage = this.rainFallRepository.findAllBySearch(pageable, deviceId, search);
            }
    ...
    public WebResponse getAvarageOneHourData(Long deviceId){
        try {
            List<RainFall> rainFalls = this.rainFallRepository.findAvarageOneHourData(deviceId);
            if (!rainFalls.isEmpty()) {
                Double sum = 0.0;
                Double avarage = 0.0;
                Integer countNull = 0;
                for (RainFall data : rainFalls) {
                ...
                avarage = sum / (rainFalls.size()- countNull);
