...
import com.tanahkube.entity.WindSpeed;
...
import com.tanahkube.repository.WindSpeedRepository;
...
public class WindSpeedService {
    ...
    public WindSpeedService(WindSpeedRepository windSpeedRepository, DeviceRepository deviceRepository){
    ...
    private DataResponse toDataResponse(WindSpeed data){
    ...
    public WebResponse create(String apiKey, DataRequest request, Errors errors){
        try {
            ...
            WindSpeed windSpeedSave = new WindSpeed();
            windSpeedSave.setValue(request.getValue());
            windSpeedSave.setDeviceId(request.getDeviceId());
            windSpeedSave.setCreatedOn(new Date());
            windSpeedSave.setIsDelete(false);
            windSpeedSave = this.windSpeedRepository.save(windSpeedSave);

            Device device = this.deviceRepository.findById(windSpeedSave.getDeviceId()).orElse(null);
            if (device != null && apiKey.equals(device.getApiKey())) {
                windSpeedSave.setDevice(device);
            }

            DataResponse dataResponse = toDataResponse(windSpeedSave);
    ...
    public WebResponse getLastData(Long deviceId){
        try {
            WindSpeed windSpeed = this.windSpeedRepository.findLastData(deviceId).orElse(null);
            if (windSpeed != null) {
                DataResponse dataResponse = toDataResponse(windSpeed);
    ...
    public WebResponse getLimitData(Long deviceId, Integer limit){
        try {
            List<WindSpeed> windSpeed = this.windSpeedRepository.findLimitData(deviceId, limit);
            if (!windSpeed.isEmpty()) {
                List<DataResponse> dataResponse = windSpeed.stream().map(this::toDataResponse).toList();
                return toWebResponse("success", "success fetch data", dataResponse, new HashMap<>());
            }else{
                return toWebResponse("success", "no data", new HashMap<>(), new HashMap<>());
            }
    ...
    public WebResponse getAllData(Long deviceId, String search, String order, Integer pageSize, Integer pageNumber){
        try {
            ...
            Page<WindSpeed> dataPage = this.windSpeedRepository.findAllData(pageable, deviceId);
            if (!search.trim().equals("")) {
                dataPage = this.windSpeedRepository.findAllBySearch(pageable, deviceId, search);
            }
    ...
    public WebResponse getAvarageOneHourData(Long deviceId){
        try {
            List<WindSpeed> windSpeeds = this.windSpeedRepository.findAvarageOneHourData(deviceId);
            if (!windSpeeds.isEmpty()) {
                Double sum = 0.0;
                Double avarage = 0.0;
                Integer countNull = 0;
                for (WindSpeed data : windSpeeds) {
                ...
                avarage = sum / (windSpeeds.size()- countNull);
