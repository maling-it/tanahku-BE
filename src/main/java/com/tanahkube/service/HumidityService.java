...
import com.tanahkube.repository.HumidityRepository;
...
public class HumidityService {
    ...
    public HumidityService(HumidityRepository humidityRepository, DeviceRepository deviceRepository){
    ...
    private DataResponse toDataResponse(Humidity data){
    ...
    public WebResponse create(String apiKey, DataRequest request, Errors errors){
        try {
            ...
            humidity.setValue(request.getValue());
            humidity.setDeviceId(request.getDeviceId());
            humidity.setCreatedOn(new Date());
            humidity.setIsDelete(false);
            humidity = this.humidityRepository.save(humidity);

            Device device = this.deviceRepository.findById(humidity.getDeviceId()).orElse(null);
            if (device != null && apiKey.equals(device.getApiKey())) {
                humidity.setDevice(device);
            }

            DataResponse dataResponse = toDataResponse(humidity);
    ...
    public WebResponse getLastData(Long deviceId){
        try {
            Humidity humidity = this.humidityRepository.findLastData(deviceId).orElse(null);
            if (humidity != null) {
                DataResponse dataResponse = toDataResponse(humidity);
    ...
    public WebResponse getLimitData(Long deviceId, Integer limit){
        try {
            List<Humidity> humidity = this.humidityRepository.findLimitData(deviceId, limit);
            if (!humidity.isEmpty()) {
                List<DataResponse> dataResponse = humidity.stream().map(this::toDataResponse).toList();
                return toWebResponse("success", "success fetch data", dataResponse, new HashMap<>());
            }else{
                return toWebResponse("success", "no data", new HashMap<>(), new HashMap<>());
            }
    ...
    public WebResponse getAllData(Long deviceId, String search, String order, Integer pageSize, Integer pageNumber){
        try {
            ...
            Page<Humidity> dataPage = this.humidityRepository.findAllData(pageable, deviceId);
            if (!search.trim().equals("")) {
                dataPage = this.humidityRepository.findAllBySearch(pageable, deviceId, search);
            }
    ...
    public WebResponse getAvarageOneHourData(Long deviceId){
        try {
            List<Humidity> humidities = this.humidityRepository.findAvarageOneHourData(deviceId);
            if (!humidities.isEmpty()) {
                Double sum = 0.0;
                Double avarage = 0.0;
                Integer countNull = 0;
                for (Humidity data : humidities) {
                ...
                avarage = sum / (humidities.size()- countNull);
