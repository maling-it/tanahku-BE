...
import com.tanahkube.repository.AirTemperatureRepository;
...
public class AirTemperatureService {
    ...
    public AirTemperatureService(AirTemperatureRepository airTemperatureRepository, DeviceRepository deviceRepository){
    ...
    private DataResponse toDataResponse(AirTemperature data){
    ...
    // Create---------
    @Transactional
    public WebResponse create(String apiKey ,@Valid DataRequest request, Errors errors){
        try {
            // Validation
            ...
            airTemperature.setValue(request.getValue());
            airTemperature.setDeviceId(request.getDeviceId());
            airTemperature.setCreatedOn(new Date());
            airTemperature.setIsDelete(false);
            airTemperature = this.airTemperatureRepository.save(airTemperature);

            Device device = this.deviceRepository.findById(airTemperature.getDeviceId()).orElse(null);
            if (device != null && apiKey.equals(device.getApiKey())) {
                airTemperature.setDevice(device);
            }

            DataResponse dataResponse = toDataResponse(airTemperature);
    ...
    public WebResponse getLastData(Long deviceId){
        try {
            AirTemperature airTemperature = this.airTemperatureRepository.findLastData(deviceId).orElse(null);
    ...
    public WebResponse getLimitData(Long deviceId, Integer limit){
        try {
            List<AirTemperature> airTemperatures = this.airTemperatureRepository.findLimitData(deviceId, limit);
            if (!airTemperatures.isEmpty()) {
                List<DataResponse> dataResponse = airTemperatures.stream().map(this::toDataResponse).toList();
                return toWebResponse("success", "success fetch data", dataResponse, new HashMap<>());
            }else{
                return toWebResponse("success", "no data", new HashMap<>(), new HashMap<>());
            }
    ...
    public WebResponse getAllData(Long deviceId, String search, String order, Integer pageSize, Integer pageNumber){
        try {
            ...
            Page<AirTemperature> dataPage = this.airTemperatureRepository.findAllData(pageable, deviceId);
            if (!search.trim().equals("")) {
                dataPage = this.airTemperatureRepository.findAllBySearch(pageable, deviceId, search);
            }
    ...
    public WebResponse getAvarageOneHourData(Long deviceId){
        try {
            List<AirTemperature> airTemperatures = this.airTemperatureRepository.findAvarageOneHourData(deviceId);
            if (!airTemperatures.isEmpty()) {
                Double sum = 0.0;
                Double avarage = 0.0;
                Integer countNull = 0;
                for (AirTemperature data : airTemperatures) {
                ...
                avarage = sum / (airTemperatures.size()- countNull);
