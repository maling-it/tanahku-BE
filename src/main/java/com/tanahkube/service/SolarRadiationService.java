...
import com.tanahkube.repository.SolarRadiationRepository;

@Service
public class SolarRadiationService {
    private SolarRadiationRepository solarRadiationRepository;
    private DeviceRepository deviceRepository;

    public SolarRadiationService(SolarRadiationRepository solarRadiationRepository, DeviceRepository deviceRepository){
    ...
    private DataResponse toDataResponse(SolarRadiation data){
    ...
    public WebResponse create(String apiKey ,DataRequest request, Errors errors){
        try {
            ...
            solarRadiation.setValue(request.getValue());
            solarRadiation.setDeviceId(request.getDeviceId());
            solarRadiation.setCreatedOn(new Date());
            solarRadiation.setIsDelete(false);
            solarRadiation = this.solarRadiationRepository.save(solarRadiation);

            Device device = this.deviceRepository.findById(solarRadiation.getDeviceId()).orElse(null);
            if (device != null && apiKey.equals(device.getApiKey())) {
                solarRadiation.setDevice(device);
            }

            DataResponse dataResponse = toDataResponse(solarRadiation);
    ...
    public WebResponse getLastData(Long deviceId){
        try {
            SolarRadiation solarRadiation = this.solarRadiationRepository.findLastData(deviceId).orElse(null);
    ...
    public WebResponse getLimitData(Long deviceId, Integer limit){
        try {
            List<SolarRadiation> solarRadiation = this.solarRadiationRepository.findLimitData(deviceId, limit);
            if (!solarRadiation.isEmpty()) {
                List<DataResponse> dataResponse = solarRadiation.stream().map(this::toDataResponse).toList();
                return toWebResponse("success", "success fetch data", dataResponse, new HashMap<>());
            }else{
                return toWebResponse("success", "no data", new HashMap<>(), new HashMap<>());
            }
    ...
    public WebResponse getAllData(Long deviceId, String search, String order, Integer pageSize, Integer pageNumber){
        try {
            ...
            Page<SolarRadiation> dataPage = this.solarRadiationRepository.findAllData(pageable, deviceId);
            if (!search.trim().equals("")) {
                dataPage = this.solarRadiationRepository.findAllBySearch(pageable, deviceId, search);
            }
    ...
    public WebResponse getAvarageOneHourData(Long deviceId){
        try {
            List<SolarRadiation> solarRadiations = this.solarRadiationRepository.findAvarageOneHourData(deviceId);
            if (!solarRadiations.isEmpty()) {
                Double sum = 0.0;
                Double avarage = 0.0;
                Integer countNull = 0;
                for (SolarRadiation data : solarRadiations) {
                ...
                avarage = sum / (solarRadiations.size()- countNull);
